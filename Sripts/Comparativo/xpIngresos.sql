...
BEGIN
	...
	SELECT 1
			,'B'
			 ...
			 ,cc.sucursal
			 ,cc.centrocostos
			 ,IIF(Cargos=0,SUM(Abonos),SUM(Cargos-Abonos)*-1) AS 'saldo'
	...
	GROUP BY a.Empresa,c.Cuenta,c.Descripcion,a.Ejercicio,a.Periodo,cc.Sucursal,cc.CentroCostos,a.Cargos
	...
	SELECT 1
			 ...
			 ,c.Descripcion
			  ...
			  ,cc.sucursal
			  ,cc.centrocostos
			  ,IIF(Cargos=0,SUM(Abonos),sum(Cargos-Abonos)) AS 'saldo'
			  ,'INGRESOS OPERATIVOS' AS 'Titulo'
	FROM Acum AS a
	JOIN Cta AS c ON c.Cuenta = a.Cuenta
	JOIN @CCostos AS cc ON cc.CentroCostos=a.SubCuenta
	...
	GROUP BY A.Empresa,a.Cuenta,c.Cuenta,c.Descripcion,a.Ejercicio,a.Periodo,cc.Sucursal,cc.CentroCostos,a.Cargos
	...
	SELECT 4
		  ...
		  ,IIF(dbo.fnJalesContabilidad(a.Cuenta)='Intereses y gastos bancarios',(Cargos-Abonos)*-1,(Cargos-Abonos)) AS 'saldo'
